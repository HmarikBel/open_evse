...
#ifdef CHARGING_REG
	pinChargingOut(&pinCharging, 1);
#endif

#ifdef CHARGING2_REG
	pinChargingOut(&pinCharging2, 1);
#endif // CHARGING2_REG
...
#else // !RELAY_AUTO_PWM_PIN
#ifdef CHARGING_REG
	pinChargingOut(&pinCharging, 0);
#endif
#ifdef CHARGING2_REG
	pinChargingOut(&pinCharging2, 0);
#endif
#ifdef CHARGINGAC_REG
 ...
 uint8_t tmppilotstate = EVSE_STATE_UNKNOWN;
  ...
  if (((m_EvseState == EVSE_STATE_C) && (m_CurrentScaleFactor > 0)) || AmmeterCalEnabled()) {
    ...
    //  m_ChargingCurrent = ma * m_CurrentScaleFactor - m_AmmeterCurrentOffset;  // subtract it
	  ...
	  m_ChargingCurrent = CustomProcessing.m_c * 1000 - m_AmmeterCurrentOffset;
